...
export async function getWeatherData(id: string): Promise<WeatherData> {
  ...
  for (let attempt = 1; attempt <= maxRetries; attempt++) {
    try {
      const resp = await fetch(
        `http://www.weather.com.cn/weather1d/${id}.shtml`,
      );
      ...
      if (nodeList.length !== 2) {
        throw new Error("Failed to find today's weather");
      }
      ...
      console.warn(
        `Attempt ${attempt}/${maxRetries} failed for weather data ${id}: ${lastError.message}`,
      );
  ...
  throw new Error(
    `Failed to get weather data for ${id} after ${maxRetries} attempts: ${lastError?.message}`,
  );
}
...
export async function getRealtimeData(id: string): Promise<RealtimeData> {
  ...
  for (let attempt = 1; attempt <= maxRetries; attempt++) {
    try {
      const response = await fetch(
        `http://d1.weather.com.cn/sk_2d/${id}.html`,
        {
          headers: {
            "User-Agent":
      ...
      lastError = error as Error;
      console.warn(
        `Attempt ${attempt}/${maxRetries} failed for ${id}: ${lastError.message}`,
      );
  ...
  throw new Error(
    `Failed to get realtime data for ${id} after ${maxRetries} attempts: ${lastError?.message}`,
  );
}
